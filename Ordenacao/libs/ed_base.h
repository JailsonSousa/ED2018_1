...
public:
    dsLine(const sf::Vector2f& point1, const sf::Vector2f& point2,
           float thickness = 1.0, sf::Color color = sf::Color::Red):
...
#define my_window (MyWindow::get_instance())
...
private:
    ...
    MyWindow():
        sf::RenderWindow(sf::VideoMode(largura, altura), "QXCODE ED")
    {
    ...
    virtual void show(){
        ...
        while(my_window->isOpen()){
            ...
            if(autoplay)
                walk(jump);
            process_input();
            my_window->clear(color_back);
            auto sprite = sf::Sprite(_buffer.get_it());
            auto wsize = my_window->getSize();
...
private:
    ...
    void process_input(){
        ...
        while(my_window->pollEvent(evt)){
            my_window->processar_close_resize(evt);
            if(evt.type == sf::Event::KeyPressed){
                if(evt.key.code == sf::Keyboard::Right){walk(jump);}
                else if(evt.key.code == sf::Keyboard::Left){walk(-jump);}
                else if(evt.key.code == sf::Keyboard::Return){autoplay = !autoplay;}
                else if(evt.key.code == sf::Keyboard::Up){jump *= 2;}
...
//        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Down)){walk(-(jump * jump));}
    ...
    void print_label(){
        ...
        if(_finished)
            ...
            state += "(Processando)";
        my_window->draw(sfText(sf::Vector2f(0, 20), state, color_front));
        ...
        auto dim = my_window->getSize();
        float width = sfText(sf::Vector2f(0, 0), title_right).getLocalBounds().width;

        my_window->draw(sfText(sf::Vector2f(dim.x - width - 10, 0), title_right, color_front));
        my_window->draw(sfText(sf::Vector2f(dim.x - width - 10, 15), teclas, color_front));

        sf::Color colorAuto = sf::Color::Red;
        if(autoplay)
            colorAuto = sf::Color::Green;
        my_window->draw(sfText(sf::Vector2f(dim.x - width - 10, 15), autoOp, colorAuto));
        my_window->draw(dsLine(sf::Vector2f(0, 40), sf::Vector2f(dim.x, 40), 1, color_front));
    }
